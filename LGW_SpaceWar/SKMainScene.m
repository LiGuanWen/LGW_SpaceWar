...
- (void)initBackground{
    ...
    _background1 = [SKSpriteNode spriteNodeWithTexture:[SKSharedAtles textureWithType:SKTextureTypeBackground] size:CGSizeMake(self.size.width, self.size.height)];
    _background1.position = CGPointMake(self.size.width / 2, 0);
    _background1.anchorPoint = CGPointMake(0.5, 0);
    _background1.zPosition = 0;
    
    _background2 = [SKSpriteNode spriteNodeWithTexture:[SKSharedAtles textureWithType:SKTextureTypeBackground] size:CGSizeMake(self.size.width, self.size.height)];
...
- (void)scrollBackground{
    ...
    [_background1 setPosition:CGPointMake(self.size.width / 2, _adjustmentBackgroundPosition - self.size.height)];
...
//            NSLog(@"游戏结束：%@",_lastScore);
            //保存最大值 和所用时间
            NSString *maxScore = [[NSUserDefaults standardUserDefaults] objectForKey:@"maxScore"];
            double lastCount = _lastScore.doubleValue;
            double maxCount = maxScore.doubleValue;
            
            if (lastCount > maxCount) {
                //保存历史最高分数和时间
                NSString *userTimes = [[NSUserDefaults standardUserDefaults] objectForKey:@"useTimes"];
                NSDictionary *dict = @{@"score":_lastScore,
                                       @"time":userTimes};
                [[NSUserDefaults standardUserDefaults] setObject:dict forKey:@"AllTimeHigh"];
